...
class Frutas {
  ...
  mostrarinfo() {
    console.log(
      `la fruta ${this.nombre}, es de color ${this.color} y hay ${this.cantidad} unidades.`
    );
  }
  agregar(n) {
    this.cantidad += n;
    console.log(
      `la fruta ${this.nombre},  ahora hay ${this.cantidad} unidades.`
    );
  }
  quitar(n) {
    this.cantidad -= n;
...
const textoFrutas = arrayDeFrutas
  .map((f) => `Nombre: ${f.nombre} Color: ${f.color} Cantidad: ${f.cantidad}`)
  .join("\n");
/*
fruta1.mostrarinfo();
fruta1.agregar(5);
